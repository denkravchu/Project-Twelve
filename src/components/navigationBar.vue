        ...
        <div class="navigation-bar__offset-container">
            <div class="navigation-bar__flow-container" :style="{ transform: `translate3d(${navbarTranslate}px, 0, 0)` }">
                ...
                <div v-for="(navButton, idx) in navButtons" 
                    @click="this.$emit('openScreen', setOpenScreenIdx(idx))"
                    :key="idx" 
                    class="nav-button"
                    :class="{
                        'active': navbarActiveScreenNumber === idx, 
                    }"
                    :ref="navButton"
...
export default {
    ...
    data() {
        return {
            ...
            navbarActiveScreenNumber: 0,
        }
    },
    created() {
        this.$watch('activeScreenNumber', () => {
            this.animateNavbarTranslate()
            this.calcNavbarActiveScreenMobile()
        })
    },
    mounted() {
    ...
    methods: {
        ...
        // bad code for mobile adaptive (fucking designer)
        setOpenScreenIdx(idx) {
            if (window.innerWidth <= this.adaptivePointSm && idx > 1 && idx < 9) {
              console.log(idx + 1)
              return idx + 1
            }
            if (window.innerWidth <= this.adaptivePointSm && idx >= 9) {
              console.log(idx + 2)
              return idx + 2
            }
            return idx
